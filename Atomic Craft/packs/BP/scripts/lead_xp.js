...
class BlockExperienceRewardComponent {
    onPlayerBreak(ev, param) {
        const { block, dimension, player } = ev;
        const params = param.params;
        // Check the tool in the player's hand
        const equippable = player?.getComponent("minecraft:equippable");
        if (!equippable)
            return; // Exit if the player or its equipment are undefined
        ...
        if (silkTouch)
            return; // Exit if the iron pickaxe has the Silk Touch enchantment
        // Spawn the XP orbs
...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent("atomic:experience_reward", new BlockExperienceRewardComponent);
});
//# sourceMappingURL=lead_xp.js.map