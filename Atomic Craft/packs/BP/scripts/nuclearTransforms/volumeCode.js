...
import { system, world, BlockVolume, Dimension, Vector3, Block, } from "@minecraft/server";
...
export async function nuclearArea(dimensionid, location, blocky, size, change) {
    ...
    for (let x = startx; x < size; x += 16) {
        for (let z = startz; z < size; z += 16) {
            ...
            const blocklist = dimension.getBlocks(blockvolum, {
                excludeTypes: [
                    "minecraft:air",
                    "minecraft:water",
                    "minecraft:lava",
            ...
            const blockGet = dimension.getBlocks(blockvolum, {
                includeTypes: [
                    "minecraft:jungle_leaves",
                    "minecraft:azalea_leaves",
                    "minecraft:oak_leaves",
