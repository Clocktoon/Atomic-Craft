import { Entity, ItemStack, Player, Vector3, world } from "@minecraft/server";
import { CustomForm, ModalFormData, Observable } from "@minecraft/server-ui";

function travelSystem(x: number, y: number, z: number, name: string, entity: Entity, player: Player) {
...
world.afterEvents.playerInteractWithEntity.subscribe((ev) => {
  ...
  if (entity.typeId === "atomic:ballistic_missile") {
    const xOb = Observable.create<string>("Xcord", {clientWritable: true})
    const yOb = Observable.create<string>("Ycord")
    const zOb = Observable.create<string>("Zcord", {clientWritable: true})
    const form = CustomForm.create(player,"Missile Control Panel")
    form.textField("X", xOb,)
    .textField("Y", yOb)
    .textField("Z", zOb)
    .closeButton()
    .divider()
