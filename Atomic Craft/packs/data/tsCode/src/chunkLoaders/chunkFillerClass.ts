...
export interface ChunkFillRequest {
  ...
  block: Block | Entity;
  name: string;
  /** Phase now travels WITH the request instead of being baked into the
 ...
 */
export function runJobAsync(gen: Generator<void, void, void>): Promise<void> {
...
class ChunkFiller {
  ...
  #currentGenerator?: Generator<void, void, void>;
  ...
  request(
    ...
    if (!this.#currentGenerator) this.#currentGenerator = this.generator();
  ...
  *generator(): Generator<void, void, void> {
    ...
    const powerful = world.getDynamicProperty("powerful") as boolean;

    
    const slab = !powerful ? 16 : 64;

    const log = (msg: string) => {
      if (logs) world.sendMessage(msg);
    };
    ...
    while (this.requests.length > 0) {
      ...
      if (request.phase === 2) {
        ...
        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "atomic:radiation_diamond_block", { includeTypes: DIAMOND_ORE });

        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "minecraft:air", { includeTypes: [...PLANTS, ...VAPORISED] });

        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "atomic:burned_log", { includeTypes: UNTAGGED_LOGS, includeTags: ["log"] });

          
        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "atomic:radiation_plank", { includeTypes: ["minecraft:planks"] });

        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "atomic:radiation_slab", { includeTypes: ["minecraft:wooden_slab"] });

        // Bulk pass last: everything still standing becomes slag.
        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "atomic:radiation_block", { excludeTypes: BULK_EXCLUDE, excludeTags: ["log"] });
        
      } else {
        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "minecraft:air", { includeTypes: PLANTS, includeTags: ["minecraft:crop", "plant"] });

        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "atomic:radi_leave", { includeTypes: LEAVES });

        touched += yield* fillSliced(dim, min, max, slab, powerful, log,
          "atomic:dead_grass", { includeTypes: GROUND_COVER });
      ...
      if (touched === 0) log(`No blocks found in volume for ${request.name}`);
      else log(`§a${request.name}: ${touched} blocks changed (phase ${request.phase})`);
...
function* scatterFire(
  ...
  try {
    const wood = dim.getBlocks(
      new BlockVolume(min, max),
      { includeTypes: ["minecraft:planks", "minecraft:wooden_slab"] },
      true,
    );
    ...
    for (const loc of wood.getBlockLocationIterator()) {
      ...
      lit++;
      if ((lit & 31) === 0) yield; // yield every 32 placements, not every one
    }
  } catch (err) {
