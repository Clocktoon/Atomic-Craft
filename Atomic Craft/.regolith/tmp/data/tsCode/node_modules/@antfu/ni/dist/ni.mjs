...
import { A as require_prompts, T as formatPackageWithUrl, h as getCatalog, i as runCli, l as parseNi, o as getCommand, w as exclude } from "./src-Ce4jhUqC.mjs";
...
//#endregion
//#region node_modules/.pnpm/pnpm-workspace-yaml@1.6.0/node_modules/pnpm-workspace-yaml/dist/index.mjs
...
function parsePnpmWorkspaceYaml(content) {
	let document = (0, import_dist.parseDocument)(content);
	let hasChanged = false;
	function setContent(newContent) {
		content = newContent;
		document = (0, import_dist.parseDocument)(content);
		hasChanged = false;
	}
	function setPath(path, value) {
		...
		if (!pair) {
			...
			pair = new import_dist.Pair(key, value);
			map.items.splice(index, 0, pair);
			hasChanged = true;
		} else if ((0, import_dist.isAlias)(pair.value)) {
...
function findAnchor(doc, alias) {
	let anchor;
	(0, import_dist.visit)(doc, { Scalar: (_key, scalar, _path) => {
...
//#endregion
//#region src/catalog/pnpm.ts
function findPnpmWorkspaceYaml(cwd) {
...
const pnpmCatalogProvider = {
	async detect(cwd) {
		const filePath = findPnpmWorkspaceYaml(cwd);
		if (!filePath) return null;
		const json = parsePnpmWorkspaceYaml(fs.readFileSync(filePath, "utf-8")).toJSON();
		const catalogs = [];
		...
		if (!hasDefaultCatalog && !hasNamedCatalogs) return null;
		if (hasDefaultCatalog) catalogs.push({
			name: "default",
			packages: json.catalog
		});
		if (hasNamedCatalogs) for (const [name, packages] of Object.entries(json.catalogs)) catalogs.push({
			name,
			packages
		});
		return {
	...
	findPackage(config, pkgName) {
		return config.catalogs.find((c) => pkgName in c.packages);
	},
	async addPackage(config, catalogName, pkgName, version) {
		const workspace = parsePnpmWorkspaceYaml(fs.readFileSync(config.filePath, "utf-8"));
		...
		if (existing) existing.packages[pkgName] = version;
		else config.catalogs.push({
			name: catalogName,
...
//#endregion
//#region src/catalog/detect.ts
function getCatalogProvider(agent) {
	if (agent === "pnpm") return pnpmCatalogProvider;
	return null;
}
//#endregion
//#region src/catalog/package-json.ts
function findClosestPackageJson(cwd) {
	let dir = path.resolve(cwd);
	while (true) {
...
function updatePackageJsonCatalogRefs(pkgJsonPath, entries, depType) {
	const content = fs.readFileSync(pkgJsonPath, "utf-8");
	const indent = detectIndent(content);
	const data = JSON.parse(content);
	if (!data[depType]) data[depType] = {};
	for (const entry of entries) data[depType][entry.name] = entry.catalogRef;
	fs.writeFileSync(pkgJsonPath, `${JSON.stringify(data, null, indent)}\n`);
}
//#endregion
//#region src/catalog/prompt.ts
const SKIP = "__skip__";
const CREATE_NEW = "__create_new__";
async function promptSelectCatalog(config, pkgName, programmatic) {
	if (config.hasDefaultCatalog && !config.hasNamedCatalogs) return { catalogName: "default" };
	if (programmatic) return { catalogName: void 0 };
	const catalogChoices = config.catalogs.map((c) => ({
		title: c.name,
		value: c.name
	}));
	const { catalog } = await (0, import_prompts.default)({
		type: "select",
		name: "catalog",
		...
		choices: [
			...catalogChoices,
			{
				title: c.dim("create new catalog"),
				value: CREATE_NEW
			},
			{
				title: c.dim("skip (install without catalog)"),
				value: SKIP
			}
		]
	});
	if (catalog === void 0 || catalog === SKIP) return { catalogName: void 0 };
	if (catalog === CREATE_NEW) return { catalogName: await promptNewCatalogName() };
	return { catalogName: catalog };
}
async function promptNewCatalogName() {
...
function getCatalogRef(catalogName) {
	return catalogName === "default" ? "catalog:" : `catalog:${catalogName}`;
}
//#endregion
//#region src/catalog/handler.ts
function splitPackagesAndFlags(args) {
	const packages = [];
	const flags = [];
	for (const arg of args) if (arg.startsWith("-")) flags.push(arg);
...
function getDepType(flags) {
	if (flags.includes("-D") || flags.includes("-d")) return "devDependencies";
	if (flags.includes("--save-peer")) return "peerDependencies";
	return "dependencies";
}
async function resolveVersion(pkgName) {
	return `^${(await getLatestVersion(pkgName)).version}`;
}
async function resolveCatalogForPackage(provider, config, pkgName, programmatic) {
	const existing = provider.findPackage(config, pkgName);
	if (existing) return { catalogName: existing.name };
	const { catalogName } = await promptSelectCatalog(config, pkgName, programmatic);
	if (!catalogName) return { catalogName: void 0 };
	return {
		catalogName,
		version: await resolveVersion(pkgName)
	};
}
async function handleCatalogInstall(agent, args, ctx) {
	if (!await getCatalog()) return void 0;
	const provider = getCatalogProvider(agent);
	if (!provider) return void 0;
	const hasWorkspaceFlag = args.includes("-w") || args.includes("--workspace");
	const { packages, flags } = splitPackagesAndFlags(args.filter((a) => a !== "-w" && a !== "--workspace"));
	...
	for (const pkg of packages) {
		...
		if (result.catalogName) {
			if (result.version) {
				...
				if (!ctx?.programmatic) console.log(`${c.green("+")} ${c.cyan(pkg)} ${c.dim(`→ ${result.catalogName} catalog (${result.version})`)}`);
			...
			catalogEntries.push({
				name: pkg,
				catalogRef: getCatalogRef(result.catalogName)
			});
		} else skippedPackages.push(pkg);
	}
	if (catalogEntries.length === 0) return void 0;
	let pkgJsonPath;
	if (hasWorkspaceFlag) pkgJsonPath = path.join(path.dirname(config.filePath), "package.json");
	else pkgJsonPath = findClosestPackageJson(cwd);
	if (!pkgJsonPath) {
		if (!ctx?.programmatic) {
	...
	if (skippedPackages.length > 0) return getCommand(agent, "add", [...skippedPackages, ...flags]);
	return getCommand(agent, "install", []);
}
//#endregion
//#region src/fetch.ts
async function fetchNpmPackages(pattern) {
	const registryLink = (pattern) => `https://registry.npmjs.com/-/v1/search?text=${pattern}&size=35`;
	const terminalColumns = process.stdout?.columns || 80;
	try {
		return (await fetch(registryLink(pattern)).then((res) => res.json())).objects.map(({ package: pkg }) => ({
			title: formatPackageWithUrl(`${pkg.name.padEnd(30, " ")} ${c.blue`v${pkg.version}`}`, pkg.links.repository ?? pkg.links.npm, terminalColumns),
			value: pkg
		}));
	} catch {
...
//#endregion
//#region src/commands/ni.ts
runCli(async (agent, args, ctx) => {
	if (args[0] === "-i") {
		let fetchPattern;
		if (args[1] && !args[1].startsWith("-")) fetchPattern = args[1];
		else {
			const { pattern } = await (0, import_prompts.default)({
				type: "text",
				name: "pattern",
		...
		const { dependency } = await (0, import_prompts.default)({
			type: "autocomplete",
			name: "dependency",
			choices: packages,
			instructions: false,
	...
	if (!args.includes("-g")) {
		...
		if (catalogCmd !== void 0) return catalogCmd;
	}
