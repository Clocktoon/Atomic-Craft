import {BlockCustomComponent, system, BlockComponentRedstoneUpdateEvent, BlockComponentTickEvent, CustomComponentParameters, BlockComponentPlayerInteractEvent, EntityComponentTypes, Player, ItemStack, EntityInventoryComponent} from "@minecraft/server"
import { CustomForm, ObservableNumber, ObservableString, ObservableUIRawMessage } from "@minecraft/server-ui"
...
class EnricherComp implements BlockCustomComponent {
    ...
    onPlayerInteract(event: BlockComponentPlayerInteractEvent) {
        ...
        if(block.permutation.getState("atomic:enrich_on" as keyof BlockStateSuperset) !== true)
          return;
         ...
         */
         function enrichLoading(item: string, time: number, form: CustomForm, slot: number, 
          invetory: EntityInventoryComponent, amount: number) {
          form.close()
          system.run(() => {
