...
function isPathClear(center: Vector3, location: Vector3, dimension: Dimension) {
  if (!center || !location || !dimension) return false;
...
function createLaunchingSmokeTrails(center: Vector3, dimension: Dimension, count = 15, launchPower = 0.5) {
...
function spawnSmokeTrajectory(center: Vector3, dimension: Dimension, angle: number, verticalAngle: number, maxDistance: number, maxHeight: number, power: number) {
