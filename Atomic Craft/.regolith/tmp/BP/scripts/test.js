...
function playExplosionAudio(dimension, center, magnitude) {
    ...
    players.forEach(player => {
        ...
        system.runTimeout(() => {
            try {
                dimension.runCommand(`playsound ${boomSound} ${player.name} ${playerLocation.x} ${playerLocation.y} ${playerLocation.z} ${boomVolume} ${boomPitch}`);
...
async function ensureChunkIsLoaded(options, tickingAreaCode) {
    if (world.tickingAreaManager.hasCapacity(options))
        return await executeTickingArea(options, tickingAreaCode);
...
async function executeTickingArea(options, tickingAreaCode) {
    ...
    await world.tickingAreaManager.createTickingArea(tickingAreaId, options);
