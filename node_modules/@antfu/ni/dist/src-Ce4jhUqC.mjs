...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/lib/elements/multiselect.js
...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/lib/elements/autocomplete.js
...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/lib/elements/autocompleteMultiselect.js
...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/lib/elements/confirm.js
...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/lib/elements/index.js
...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/lib/prompts.js
...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/lib/index.js
...
//#endregion
//#region node_modules/.pnpm/@posva+prompts@2.4.4/node_modules/@posva/prompts/index.js
...
async function openTemp() {
	if (!existsSync(CLI_TEMP_DIR)) await promises.mkdir(CLI_TEMP_DIR, { recursive: true });
	...
	return promises.open(competitivePath, "wx").then((fd) => ({
		...
		cleanup() {
			fd.close().then(() => {
				if (existsSync(competitivePath)) promises.unlink(competitivePath);
			});
		}
	})).catch((error) => {
...
async function writeFileSafe(path, data = "") {
	...
	if (temp) promises.writeFile(temp.path, data).then(() => {
		const directory = dirname(path);
		if (!existsSync(directory)) promises.mkdir(directory, { recursive: true });
...
function terminalLink(text, url, options) {
	if (process$1.env.FORCE_HYPERLINK || process$1.stdout.isTTY && !process$1.env.NO_HYPERLINK) return `\u001B]8;;${url}\u0007${text}\u001B]8;;\u0007`;
	if (options?.fallback) return options.fallback(text, url);
	return `${text} (${url})`;
}
function formatPackageWithUrl(pkg, url, limits = 80) {
	return url ? terminalLink(pkg, url, { fallback: (_, url) => pkg.length + url.length > limits ? pkg : pkg + c.dim` - ${url}` }) : pkg;
}
//#endregion
...
async function detect$1({ autoInstall, programmatic, cwd } = {}) {
	...
	if (existsSync(path.join(targetDir, "deno.json")) || existsSync(path.join(targetDir, "deno.jsonc"))) return "deno";
	const { name, agent, version } = await detect({
		cwd,
	...
	if (name && !cmdExists(name) && !programmatic) {
		if (!autoInstall) {
			...
			if (process$1.env.CI) process$1.exit(1);
			const { tryInstall } = await (0, import_prompts.default)({
				name: "tryInstall",
				type: "confirm",
				message: `Would you like to globally install ${terminalLink(name, INSTALL_PAGE[name])}?`
			});
...
async function getConfig() {
	if (!config) {
		config = {
			...defaultConfig,
			...fs.existsSync(rcPath) ? import_ini.default.parse(fs.readFileSync(rcPath, "utf-8")) : null
		};
...
async function getDefaultAgent(programmatic) {
	...
	if (defaultAgent === "prompt" && (programmatic || process$1.env.CI)) return "npm";
	return defaultAgent;
}
...
function getCommand(agent, command, args = []) {
	if (!src_exports.COMMANDS[agent]) throw new Error(`Unsupported agent "${agent}"`);
	if (!src_exports.COMMANDS[agent][command]) throw new UnsupportedCommand({
		agent,
		command
	});
...
const parseNr = (async (agent, args, ctx) => {
	...
	if (runAgent === "node") {
		...
		if (majorNodeVersion < 22) throw new Error("The runAgent \"node\" requires Node.js 22.0.0 or higher");
		runWithNode = true;
		args = ["--run", ...args];
	}
	...
	const cmd = runWithNode ? {
		command: "node",
		args
	} : getCommand(agent, "run", args);
	if (ctx?.cwd) cmd.cwd = ctx.cwd;
	if (!cmd) return cmd;
	if (hasIfPresent && !runWithNode) cmd.args.splice(1, 0, "--if-present");
	return cmd;
});
const parseNup = ((agent, args) => {
	if (args.includes("-i")) return getCommand(agent, "upgrade-interactive", exclude(args, "-i"));
	return getCommand(agent, "upgrade", args);
});
const parseNd = ((agent, args) => {
	if (agent === "pnpm") args = args.map((i) => i === "-c" ? "--check" : i);
	if (agent === "npm") args = args.map((i) => i === "-c" ? "--dry-run" : i);
	return getCommand(agent, "dedupe", args);
});
const parseNun = ((agent, args) => {
	if (args.includes("-g")) return getCommand(agent, "global_uninstall", exclude(args, "-g"));
...
const DEFAULT_ENVIRONMENT_OPTIONS = { autoInstall: false };
...
async function runCli(fn, options = {}) {
	...
	const { args = process$1.argv.slice(2).filter(Boolean) } = options;
	try {
		await run(fn, args, options);
	} catch (error) {
		if (error instanceof UnsupportedCommand && !options.programmatic) console.log(c.red(`\u2717 ${error.message}`));
		if (!options.programmatic) process$1.exit(1);
		throw error;
	}
}
...
async function run(fn, args, options = {}) {
	...
	if (args.length === 1 && (args[0]?.toLowerCase() === "-v" || args[0] === "--version")) {
		...
		const getV = (a) => {
			const { command, args } = getCmd(a);
			return x(command, args, xVersionOptions).then((e) => e.stdout).then((e) => e.startsWith("v") ? e : `v${e}`);
		};
	...
	if (useSfw && cmdExists("sfw")) {
		...
		if (programmatic) throw new Error("sfw is enabled but not installed");
...
import * as import_package_manager_detector_commands from "package-manager-detector/commands";
__reExport(src_exports, import_package_manager_detector_commands);
import * as import_package_manager_detector_constants from "package-manager-detector/constants";
__reExport(src_exports, import_package_manager_detector_constants);
//#endregion
export { require_prompts as A, cmdExists as C, remove as D, limitText as E, terminalLink as O, CLI_TEMP_DIR as S, formatPackageWithUrl as T, getDefaultAgent as _, UnsupportedCommand as a, getUseSfw as b, parseNd as c, parseNr as d, parseNun as f, getConfig as g, getCatalog as h, runCli as i, writeFileSafe as k, parseNi as l, serializeCommand as m, getCliCommand as n, getCommand as o, parseNup as p, run as r, parseNa as s, src_exports as t, parseNlx as u, getGlobalAgent as v, exclude as w, detect$1 as x, getRunAgent as y };
